...
def get_attn_pad_mask(seq_q, seq_k):
    ...
    '''
    输入：
    dec_inputs:batc_size, tgt_len
    enc_inputs:batc_size, seq_len
    ...
    #只对k进行了padmask,没有考虑q是否mask,是因为，PAD分为对encoder自己或者decoder自己mask,此时q==k
    # 但是在decoder部分，padm,mask需要对qk这个矩阵进行，decoder时候已经做了一个自己的pad，这里面只需要再对k进行pad mask就行了
...
class Decoder(nn.Module):
    ...
    def forward(self, dec_inputs, enc_inputs, enc_outputs):
        ...
        #Xemb+pos_emb
        dec_outputs = self.pos_emb.forward(dec_outputs.transpose(0, 1)).transpose(0, 1)# [batch_size, tgt_len, tgt_emb]
        ...
        # torch.gt(a,b)：a中大于（不包含等于）b值为1，否则为0
        dec_self_attn_mask = torch.gt((dec_self_attn_pad_mask + dec_self_attn_subsequence_mask), 0)# [batch_size, tgt_len, tgt_len]

        # enc dec pad mask 在decoder的第二个attention里面使用
